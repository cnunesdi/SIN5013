...
/* Inserção AVL: p é inicializado com o endereco do nó raiz e
   *alterou com false                                         */
void inserirAVL(PONT* pp, TIPOCHAVE01 ch, bool* alterou){
	PONT p = *pp;
	if(!p){
		*pp = criarNovoNo(ch);
		*alterou = true;
...
bool excluirAVL(PONT* raiz, TIPOCHAVE01 ch, bool* alterou){
  PONT atual = *raiz;
...
void inserirAVL2(PONT *pp, TIPOCHAVE01 ch, bool *alterou){
	PONT p = *pp;
	if(!p){
		*pp = criarNovoNo(ch);
		*alterou = true;
...
void travessiaAux(PONT p, int *niv, TIPOCHAVE01 ch, bool *achou) {
	if(p) {
		(*niv)++;// *niv = *niv + 1;
...
/* Retorna true e o nivel de uma chave (caso seja encontrada) e false caso
       contrario.                                                                */
bool travessia(PONT p, int *niv, TIPOCHAVE01 ch) {
	*niv = 0;
